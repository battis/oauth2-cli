...
This [script](./src/index.ts) is identical to the [ejs-optional](../02%20ejs-optional#readme) example (authorize GitHub API access and request this repo), but a `Token.FileStorage` instance is provided to the client to store the token in a local file between runs.
