...
- [plugin-provider](./plugin-provider/src/) packages the plugin for reuse.
  - [`GitHubPlugin.ts`](./plugin-provider/src/GitHubPlugin.ts) extends `OAuth2Plugin` to be pre-configured to access the GitHub API, importing all components from the `extendable` namespace of `@oauth2-cli/qui-cli` to avoid _also_ auto-registering the default plugin instance.
