...
Shell.exec(
  `gcloud beta run services update ${service.metadata.name} ` +
    `--region=${region} ` +
    `--iap ` +
...
if (wizard) {
  Shell.exec(
    `gcloud run services add-iam-policy-binding ${service.metadata.name} ` +
      `--region=${region} ` +
...
for (const user of users) {
  Shell.exec(
    `gcloud beta iap web add-iam-policy-binding --member=user:${user} --role=roles/iap.httpsResourceAccessor --region=${region} --resource-type=cloud-run --service=${service.metadata.name} --project=${project?.projectId} --format=json --quiet`
  );
}
